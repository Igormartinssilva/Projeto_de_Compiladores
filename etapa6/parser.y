...
program:            push function_list  
{
    ...
    { 
        ...
        for(int i = 0; i < table->num_entries; i++)
        {
            ...
            {
                if(i>0){
                    cod_out = merge_code(UNIT_CODE, $2->code, cod_out );
                }
                else{
                    cod_out = $2->code;
                }
...
{   
    ...
    if($3 != NULL){
        asd_add_child($$, $3);
        iloc_list_t *cod_out = generate_code("func", $2->label, NULL,NULL);
        $$->code = merge_code(UNIT_CODE, cod_out, $3->code);
    }
};
...
return_op:              TK_PR_RETURN expr                       
{
    ...
    iloc_list_t *cod_out = generate_code("return", $2->temp, NULL, NULL);
    $$->code = merge_code(UNIT_CODE, $2->code, cod_out);
}; 
...
{
    $$ = asd_create_control_node("if", $3, $5, NULL);
    $$ = gen_and_merge_if_code($$, $3, $5);
}
|                       TK_PR_IF '(' expr ')' command_block TK_PR_ELSE  command_block  
{
    $$ = asd_create_control_node("if", $3, $5, $7);
    $$ = gen_and_merge_if_else_code($$, $3, $5, $7);
}
|                       TK_PR_WHILE '(' expr ')' command_block                
{
    $$ = asd_create_control_node("while", $3, $5, NULL);
    $$ = gen_and_merge_while_code($$, $3, $5);
};
...
expr_unary:         '-' expr_unary                          
{
    ...
    $$ = gen_and_merge_unary_code($$,"neg",$2, NULL);
}
|                   '!' expr_unary                          
{
    $$ = asd_create_with_child_and_type("!", $2, $2->type);
